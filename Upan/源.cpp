...
int i = 0;
wchar_t Spath[] = _T("F:\\move\\"), Ppath[100];
HANDLE hThread[MAX_THREADS];//线程数
//function 
...
DWORD WINAPI CopyUpan(LPVOID lpParam);
...
BOOL SearchFile(TCHAR* szPath)
{
	...
	do
	{
		...
		else
		{
			...
			wcscat(TagPath, windata.cFileName);
			/*
			创建线程
			复制文件
			写入错误信息
			*/
			CopyFile(UFPaht,TagPath,false);
		}
